...
INSERT INTO Employee3 (EmployeeID, EmployeeName, EmployeeCity, EmployeeEmail, Price)
VALUES
	...
	select sum(price) as total from Employee3 group by EmployeeCity;

	select EmployeeCity, avg(price) as total_Avg from Employee3 group by EmployeeCity;

	select EmployeeCity, sum(price) as total_sum from Employee3 group by EmployeeCity;

	select EmployeeCity, min(price) as min_price from Employee3 group by EmployeeCity;