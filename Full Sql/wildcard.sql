create table customers21(
	...
	insert into customers21(cust_id, cust_name, cust_address, cust_phone, email) values 
...
-- where the name start from the K A B C D E
select *  from customers21 where cust_name like'[KZD]%';

-- Return all the names starting betwwen the a-f

select * from customers21 where cust_name like'[a-M]%';

-- atleast 3 letters

select * from customers21 where cust_name like'a__%';

-- second position

select *  from customers21 where cust_name like '_a%';

select * from customers21 where cust_name like 'Kamran Ali';